import type { SpokenLanguage } from './providers.js';

// Deterministic TTS text optimization (Phase 4 groundwork; Phase 5 extends).
// Turns LLM/backend strings into speakable text: spoken currency, no markdown
// or symbols, capped length. Language-aware for Hinglish ("rupaye").
export function optimizeForSpeech(text: string, language: SpokenLanguage = 'english'): string {
  const moneyWord = language === 'hinglish' ? 'rupaye' : 'rupees';
  let out = text;
  ...
  out = sentences.map(s => s.trim()).filter(Boolean).slice(0, 3).join(' ');
...
// Two-sentence spoken product summary for the "haan batao" details turn.
// Currency stays numeric here; optimizeForSpeech() speaks it downstream.
export function spokenProductSummary(p: SpokenProduct, language: SpokenLanguage): string {
